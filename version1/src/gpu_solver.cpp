...
layout(std430, binding = 1) buffer ParamBuffer {
    ...
    float t_current;
    int n_equations;
    float lambda; // For exponential decay test
};
...
void main() {
    ...
    // Update state
    float y_new = y + b1 * k1 + b3 * k3 + b4 * k4 + b5 * k5 + b6 * k6;
...
void GPUSolver::solve(const ODESystem& system, 
    ...
    solution.push_back(y0); // Initial condition
    
    // Integration loop
    glUseProgram(program);
    
    for (int i = 1; i < n_steps; ++i) {
        params.t_current = static_cast<float>(t0 + (i-1) * dt);
        glBindBuffer(GL_SHADER_STORAGE_BUFFER, param_buffer);
        glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, sizeof(params), &params);
        
        // Dispatch compute shader
        GLuint work_groups = (n_equations + 63) / 64; // Round up division
        glDispatchCompute(work_groups, 1, 1);
        glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT);
        
        // Read back results
        glBindBuffer(GL_SHADER_STORAGE_BUFFER, state_buffer);
        float* result_data = static_cast<float*>(
            glMapBufferRange(GL_SHADER_STORAGE_BUFFER, 0, n_equations * sizeof(float), 
                           GL_MAP_READ_BIT));
        
        std::vector<double> step_result(n_equations);
        for (int j = 0; j < n_equations; ++j) {
            step_result[j] = static_cast<double>(result_data[j]);
        }
